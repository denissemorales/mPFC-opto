...
def NewtonSolve(x_prior, sigma_prior, N, Nmax, mu):
    ...
    for i in range(30):
        g = (
            xp
            + sp * (N - Nmax * np.exp(mu + it) / (1.0 + np.exp(mu + it)))
            - it
        )
        gprime = (
            -Nmax * sp * np.exp(mu + it) / (1.0 + np.exp(mu + it)) ** 2.0 - 1.0
        )
        x = it - g / gprime
...
def EM(xx, mu, sigma2e, x_init, sigma_init):
    ...
    while diff_its > 0.00001 and its <= max_its:
        ...
        x_T, sigma2_T, A = BackwardFilter(
            x_post, x_prior, sigma2_post, sigma2_prior
        )
