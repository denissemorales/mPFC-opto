...
class UpDownStateSelection(SpyglassMixin, dj.Manual):
    ...
    -> SortedSpikesGroup             # Population MUA
    ---
...
class UpDownStates(SpyglassMixin, dj.Computed):
    ...
    ---
    state_labels: longblob           # 0=DOWN, 1=UP per time bin (-1=outside NREM)
    ...
    def _fetch_data(self, key):
        ...
        sleep_key = {k: key[k] for k in SleepScoring.primary_key if k in key}
    ...
    def _threshold_detection(self, so_signal, mua_rate, nrem_mask, params):
        """
        DOWN = epochs where both |LFP| and MUA are below their respective
        percentile thresholds (computed within NREM only).
        """
        n = len(so_signal)
    ...
    def _gmm_detection(self, so_signal, mua_rate, nrem_mask, params):
        """
        Fit a 2-component GMM to (|LFP|, MUA) within NREM;
        DOWN = low-activity component.
        """
        ...
        states[nrem_indices] = np.where(labels == down_cluster, 0, 1)
    ...
    # ==================== Visualisation ====================
    def plot_up_down_states(self, t_start=None, t_stop=None, figsize=(15, 8)):
        """
        Plot LFP slow oscillation, MUA rate, and UP/DOWN state shading
        ...
        t_stop : float, optional
            Stop time in seconds. Defaults to first 60s.
        figsize : tuple, optional
            Figure size.
        """
        # --- Fetch stored results ---
        states, timestamps = self.fetch1("state_labels", "timestamps")
        sel = (UpDownStateSelection & self).fetch1()

        if t_start is None:
            t_start = timestamps[0]
        if t_stop is None:
            t_stop = min(timestamps[0] + 60, timestamps[-1])

        win = (timestamps >= t_start) & (timestamps <= t_stop)
        t = timestamps[win]
        s = states[win]

        # --- Fetch LFP slow oscillation ---
        so_df = (
            lfp_band.LFPBandV1
            & {
                "lfp_merge_id": sel["lfp_merge_id"],
        ...
        so_win = (so_df.index >= t_start) & (so_df.index <= t_stop)
        so_t = so_df.index[so_win].values
        so_sig = so_df[so_win].mean(axis=1).values

        # --- Fetch MUA via SortedSpikesGroup ---
        spike_indicator = SortedSpikesGroup.get_spike_indicator(
            (UpDownStateSelection & self).fetch1(), t
        )
        mua = spike_indicator.sum(axis=1)
        ...
        ax_lfp.set_title("UP / DOWN States during NREM")

        # MUA panel
        ax_mua.plot(t, mua, color="darkorange", linewidth=0.6, label="MUA")
        _shade_states(ax_mua)
        ax_mua.set_ylabel("Spike count")
        ax_mua.set_xlabel("Time (s)")
        ax_mua.legend(loc="upper right", fontsize=8)
